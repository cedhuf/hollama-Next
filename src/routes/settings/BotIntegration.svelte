<script lang="ts">
	import { Bot, Check, ChevronDown, KeyRound, LoaderCircle, Plug, X } from '@lucide/svelte';
...
<div class="border-shade-3 bg-shade-0 overflow-hidden rounded-xl border">
	<div class="relative flex items-center gap-3 p-3">
		<!-- The row opens the options, and the controls on it sit above this: a card
		     whose only target is a twelve-pixel chevron is a card people miss. Laid
		     underneath rather than wrapped around, because a heading you can rename
		     cannot live inside a button. -->
		<button
			type="button"
			onclick={() => (open = !open)}
			aria-label={$LL.botOptions()}
			aria-expanded={open}
			class="absolute inset-0"
		></button>

		<span
			class="bg-shade-2 {runs
				? 'text-positive'
				: 'text-muted'} pointer-events-none flex h-9 w-9 shrink-0 items-center justify-center rounded-lg"
			aria-hidden="true"
		>
			<Bot class="base-icon" />
		</span>
		...
		<span class="pointer-events-none flex min-w-0 flex-1 flex-col items-start">
			<!-- `field-sizing: content` is the whole trick: the field measures its own
			     text and is exactly that wide. `size` stays as the fallback for a
			     browser that does not know the property yet, and it counts in the width
			     of a zero, which is wider than an average letter: that slack on the
			     right is what the property removes. -->
			<input
			...
			<span class="text-muted flex items-center gap-1.5 text-xs">
				<span
					class="inline-block h-1.5 w-1.5 shrink-0 rounded-full {runs
						? 'bg-positive'
						: 'bg-shade-4'}"
				></span>
		...
		<!-- Whether this bot answers at all: a state you want to see and flip at a
		     glance, so it lives on the line rather than inside the form. -->
		<label
			class="relative flex shrink-0 cursor-pointer items-center"
			title={$LL.integrationEnabled()}
		>
			<input
				type="checkbox"
				bind:checked={integration.enabled}
				onchange={onChange}
