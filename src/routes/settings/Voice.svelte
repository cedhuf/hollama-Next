<script lang="ts">
	 ...
	 *
	 * Its own tab rather than a corner of Chat: two models, each with a switch and a
	 * voice by name, the language and the parameters still to come, and the loop's
	 * own behaviour underneath.
	 *
	 * `Voice` rather than `Speech`, because `speech` is the model kind that means
	 * synthesis specifically, and a tab of that name holding transcription would
	 * make one word mean two things a screen apart.
	 */
	const voiceCfg = $derived($chatDefaultsConfig.voice);
</script>

<SettingsPanel>
	<!-- Dictation. Not tied to the phone, though that is where it was built for.
	...
	<SettingsSection title={$LL.voiceInput()} description={$LL.voiceInputHelp()} card>
		{#if voiceCfg.editable}
			...
			{#if $settingsStore.voiceInput}
				<!-- Transcription models only. The picker reads the kind from the catalogue,
				     which takes the provider's word where it gives one. An empty list is the
				     honest answer: this account can reach no model that transcribes. -->
				...
				<!-- Dictation only. Reading aloud has nowhere to put a language: on Kokoro,
				     Aura and Voxtral it is part of the voice's own name, and on Gemini and Grok
				     the voices are timbres with no way to override it. -->
	...
	<!-- The other direction, and its own switch: dictating into a field is a
	     convenience, being read to is a mode and usually a second model. -->
	<SettingsSection title={$LL.speechOutput()} description={$LL.speechOutputHelp()} card>
		...
		{#if $settingsStore.speechOutput}
			<!-- A different list from the one above, not a filter of it: Kokoro answers 400
			     to a recording and Whisper answers 400 to a sentence. -->
			<SettingsField label={$LL.speechModel()}>
				<ModelSelect
					...
					onSelect={(name) => {
						...
						// The names belong to the model, so carrying one across would leave a voice
	...
	<!-- The screen's own behaviour, which belongs to neither model above: what the
	     voice screen looks like and does between one question and the next. When a
	     turn ends is no longer a setting, since it is no longer a guess. -->
