<script lang="ts">
	...
	/**
	 * The code Auth.js sent back, shown under the message. "Sign-in failed" is what
	 * somebody can act on; the code is what whoever runs the instance can act on.
	 *
	 * `Configuration` means the provider could not be built or its discovery
	 * document could not be fetched *from the server*, which looks identical from a
	 * browser that reaches the identity provider perfectly well.
	 */
