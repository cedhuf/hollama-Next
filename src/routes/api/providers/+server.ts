...
// The providers a user may use, with their available models. System servers give
// every model to an admin and the shared subset to everyone else; personal ones
// give the owner's own. Keys are never included.
	...
	const system = await Promise.all(
		listSystemServers()
			...
			.map(async (server) => {
				...
				// Display names and kinds ride along with the catalogue, so every model dropdown
				// can render and filter without a second round-trip.
				//
				// Two sources for the kinds, in order of authority: what the provider declared
				// when asked a narrow question, then what somebody stored against this
				// connection. A person's correction is always the last word.
				return {
