 ...
 *
 * Everything visual is in `Toasts.svelte` and `ToastBody.svelte`, everything
 * behavioural here. Call sites say what happened and how bad it is; they cannot
 * pass a position or a class, which is how the app ended up with red banners at
 * the top and update notices at the bottom right out of one library.
 *
...
export type ToastOptions = {
	...
	description?: string;
	/** Reuse an existing notification rather than stacking a new one: a progress line becoming a result, a retry replacing the failure it retries. */
	id?: number | string;
	/** Something to do about it. Given one, the notification waits to be read. */
	action?: { label: string; onClick: () => void };
	/** Stay until dismissed, for a state that is still true after the message goes: saving is paused, this build is out of date. Never for a plain failure. */
	persist?: boolean;
	/** Errors get it without asking, since an error is what ends up pasted into an issue. Set it where a message is worth keeping for another reason. */
	copyable?: boolean;
};

/** An error is read after the fact, so it stays around longer than a confirmation nobody needs to read. Anything carrying a button longer still: it has to survive the walk from reading to reaching for the mouse. */
const DURATION = {
	success: 4_000,
	info: 5_000,
	warning: 7_000,
...
function build(
	...
	return {
		...
		// The close button is what makes a persistent one dismissible at all, and the
...
export const toast = {
	...
	/** No duration of its own: pass its id to `success` or `error` when it ends, or it stays on screen forever. */
	loading: (message: string, opts?: ToastOptions) =>
		sonner.loading(message, build('loading', message, { ...opts, persist: true }, DURATION.info)),

	/** News rather than an outcome. No severity colour, and it waits: a notice that disappears on its own is one the user never had a chance to act on. */
