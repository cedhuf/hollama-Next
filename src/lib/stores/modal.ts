...
/**
 * The way out of a settings sub-view, lifted to the modal's own header: a
 * sub-view carrying its own back button put two headers on top of each other.
 * Registered on mount and cleared on destroy, so nothing has to tidy up.
 */
 ...
 *
 * The tour composes itself, which is right and which means nobody can look at
 * the whole thing without emptying their account. This is the developer-options
 * way in, and the steps shown are the real ones in the real order.
 */
export const welcomeShowAll = writable(false);

/** Global because it answers to Command-K from anywhere, and the sidebar hands its filter text over when the user asks for the whole corpus. */
...
/** A dialog rather than a page: a collection is written down beside what you are doing, usually without leaving the conversation that made you want to write it. */
