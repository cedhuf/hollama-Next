...
Everything above it — components, stores, features — is written once against that interface. If
...
| `src/routes/api/`     | The HTTP API — see the [reference](/llooma/reference/api/)                                        |
| `src/i18n/`           | typesafe-i18n dictionaries; English is the base, French extends it                                |
...
verified by type-checking, linting, the build, and targeted scripts — not by the suite. Fixing it
is on the roadmap and would be a genuinely valuable contribution.
:::
...
and add the path and its methods to the spec. The check compares the _surface_ — paths and
methods — not response shapes, so it will not tell you a field was renamed. That part is still on
you.

## Conventions

- Commit messages follow [Conventional Commits](https://www.conventionalcommits.org); the version
  and changelog are produced by semantic-release from them.
- Issues are for bugs. Feature requests go to
  [Discussions](https://github.com/cedhuf/llooma/discussions) first.
