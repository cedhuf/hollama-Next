...
| `MCP_ALLOWED_ORIGINS`       | _(empty)_   | Allowlist of origins an [MCP server](/connect/mcp/) may be reached at; empty = any public host, so a hub on a private address has to be named here                             |
