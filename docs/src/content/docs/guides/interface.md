...
- **Fade messages a summary has replaced.** On by default, so it is visible at a glance where the
  live context begins after a [compaction](/chat/compaction/). Hovering a faded message
...
[search](/chat/search/) over every conversation rather than filtering the titles in place.
...
it, save it as [knowledge](/context/knowledge/), copy it as Markdown or JSON, delete it. Deleting
...
Speaking rather than typing is its own subject, dictation on any screen and a hands-free loop on
this one: [Voice](/media/voice/).
